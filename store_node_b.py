...
class StoreNodeB:
    def __init__(self):
        ...
        print(f"Mengelola Cloud Database B: {self.url}")
        
        try:
            self.supabase: Client = create_client(self.url, self.key)
            self.processed_requests = set()
        except Exception as e:
            print(f"Gagal connect ke Cloud B: {e}")
            sys.exit(1)

    def process_computation(self, menu_item, agg_type):
        print(f"   > Hitung {agg_type} untuk '{menu_item}' di Cloud B...")
            ...
            try:
                ...
                if requests.data:
                    ...
                    if req_id not in self.processed_requests:
                        ...
                        print(f"   > Jawaban diposting ke Cloud B.\n")
